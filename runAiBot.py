...
try:
    ...
    print_lg("[SECURITY] Warning: Phase 3 security modules not available")
...
def login_LN() -> None:
    ...
    * If both failed, asks user to login manually
    ...
    try:
        ...
        if account_monitor:
            account_monitor.record_login(username, success=True)
        return print_lg("Login successful!")
    except Exception as e:
        # Phase 3: Log failed login attempt
        if audit_logger:
            audit_logger.log_login(username, success=False)
        if account_monitor:
            account_monitor.record_login(username, success=False)
...
def external_apply(pagination_element: WebElement, job_id: str, job_link: str, resume: str, date_listed, application_link: str, screenshot_name: str) -> tuple[bool, str, int]:
    ...
    try:
        ...
        global failed_count
        failed_count += 1
        # Phase 3: Log application error
        if account_monitor:
            account_monitor.record_error("application_error", "Failed to apply to job", {"job_id": job_id, "job_link": job_link})
        if audit_logger:
            audit_logger.log_error("application_error", "Failed to apply to external job", {"job_id": job_id, "error": str(e)})
            ...
            while current_count < switch_number:
                ...
                for job in job_listings:
                        ...
                        try: 
                            try:
                                ...
                                while next_button:
                                    ...
                                    # Phase 2: Intelligent Resume Selection
                                    if useNewResume and not uploaded:
                                        selected_resume = default_resume_path
                                        if enable_phase2_resume_selection:
                                            try:
                                                global resume_selector
                                                if resume_selector is None:
                                                    resume_selector = ResumeSelector()
                                                    print_lg("[PHASE2] Initialized Resume Intelligence system")
                                                if description != "Unknown":
                                                    selected_file, selection_info = resume_selector.select_resume(
                                                        job_description=description,
                                                        job_title=title,
                                                        company=company
                                                    )
                                                    if selected_file:
                                                        selected_resume = selected_file
                                                        print_lg(f"[PHASE2] Selected '{selection_info['resume_type']}' resume (score: {selection_info['match_score']:.2f})")
                                                    else:
                                                        print_lg("[PHASE2] No suitable resume found, using default")
                                            except Exception as e:
                                                print_lg(f"[PHASE2] Resume selection failed: {e}, using default resume")
                            ...
                            failed_count += 1
                            # Phase 3: Log easy apply error
                            if account_monitor:
                                account_monitor.record_error("easy_apply_error", "Failed to complete easy apply", {"job_id": job_id, "company": company})
                            if audit_logger:
                                audit_logger.log_error("easy_apply_error", "Failed during easy apply process", {"job_id": job_id, "error": str(e)})
                            discard_job()
                            continue
                    else:
                    ...
                    if application_link == "Easy Applied": 
                        ...
                        if audit_logger:
                            audit_logger.log_event("job_application", {"company": company, "title": title, "job_id": job_id}, "low", "system")
                    else:   
    ...
    try:
        ...
        if SECURITY_ENABLED:
            try:
                ...
                print_lg("[PHASE3] Security components initialized successfully")
            except Exception as e:
                print_lg(f"[PHASE3] Failed to initialize security components: {e}")
                audit_logger = None
                account_monitor = None
                vault = None
                secrets_manager = None
        ...
        if account_monitor:
            ...
            print_lg(f"[PHASE3] Account Health Score: {health_status.get('health_score', 'N/A')}/100")
            print_lg(f"[PHASE3] Account Status: {health_status.get('status', 'N/A')}")
            
            # Check for suspicious patterns
            suspicious = account_monitor.detect_suspicious_patterns()
            if suspicious:
                print_lg(f"[PHASE3] ⚠️ Detected {len(suspicious)} suspicious pattern(s)")
                for pattern in suspicious:
                    print_lg(f"  - {pattern['pattern']}: {pattern['description']}")
        
        if audit_logger:
            stats = audit_logger.get_statistics()
            print_lg(f"[PHASE3] Audit Log Statistics: {stats['total_events']} events logged")
